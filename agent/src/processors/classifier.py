...
{{"priority": "P2", "justification": "..."}}"""
    ...
    def _create_incident(
        ...
        if not unique_recommendations:
            unique_recommendations = self._generate_default_recommendations(
                primary_category, affected_resources, first_event
            )
        ...
        # Generate default RCA if none exists
        if not root_cause:
            root_cause = self._generate_default_rca(
                primary_category, affected_resources, first_event, events
            )
    ...
    def _generate_default_recommendations(
        ...
        recommendations = {
            IncidentCategory.AVAILABILITY: [
                ...
                "Check load balancer and auto-scaling configuration"
            ],
        ...
        return recommendations.get(category, [
            "Investigate the issue and gather more information",
            "Review recent changes to affected resources",
            "Check monitoring dashboards for related issues",
            "Consult runbooks for similar incidents"
        ])

    def _generate_default_rca(
        self,
        category: IncidentCategory,
        affected_resources: List[str],
        event: Dict[str, Any],
        ...
        """Generate default root cause analysis."""
        resource_str = ", ".join(affected_resources[:3])
        if len(affected_resources) > 3:
            resource_str += f" and {len(affected_resources) - 3} more"
        
        event_count = len(events)
        description = event.get("description", "Unknown issue")
        
        rca = f"Detected {event_count} related event(s) affecting {resource_str}. "
        rca += f"Primary issue: {description}. "
        
        if category == IncidentCategory.AVAILABILITY:
            rca += "This appears to be an availability issue that may impact service uptime."
        elif category == IncidentCategory.PERFORMANCE:
            rca += "This appears to be a performance degradation issue."
        elif category == IncidentCategory.ERROR_RATE:
            rca += "This appears to be related to increased error rates."
        elif category == IncidentCategory.RESOURCE_EXHAUSTION:
            rca += "This appears to be caused by resource exhaustion."
        elif category == IncidentCategory.SECURITY:
            rca += "This appears to be a security-related issue requiring immediate attention."
        elif category == IncidentCategory.CONFIGURATION:
            rca += "This appears to be related to configuration issues."
        elif category == IncidentCategory.CAPACITY:
            rca += "This appears to be a capacity-related issue."
        else:
            rca += "Further investigation is needed to determine the root cause."
