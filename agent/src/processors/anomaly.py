    ...
    async def analyze_events(
        ...
        for event_type, type_events in grouped.items():
            try:
                ...
                # Alarm events are inherently actionable — always treat them as anomalies
                # The AI should determine severity/RCA, not whether the alarm is "real"
                for result in analysis:
                    event_data = result.get("event", {})
                    if event_data.get("event_type") == "alarm" and event_data.get("state") == "ALARM":
                        result["is_anomaly"] = True
                        if result.get("anomaly_score") and result["anomaly_score"].score < 0.5:
                            result["anomaly_score"] = AnomalyScore(
                                score=max(0.7, result["anomaly_score"].score),
                                confidence=result["anomaly_score"].confidence,
                                reasoning=result["anomaly_score"].reasoning,
                                factors=result["anomaly_score"].factors,
                            )
