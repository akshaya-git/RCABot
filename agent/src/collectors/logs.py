...
class LogsCollector(BaseCollector):
    ...
    def _log_event_to_cloudwatch_event(
        ...
        # Skip INFO level logs - only collect actual errors/warnings
        if severity == "INFO":
            return None

        # Skip Kubernetes audit logs (they're just noise)
        if "audit.k8s.io" in message or "ResponseComplete" in message:
            return None
