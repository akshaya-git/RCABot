...
"""
        ...
        if incident.root_cause_analysis:
            message += f"\n\nRoot Cause Analysis:\n{incident.root_cause_analysis[:1000]}"

        if incident.ticket_url:
            message += f"\n\nServiceNow Ticket: {incident.ticket_url}"

        if incident.recommended_actions:
            message += "\n\nRecommended Actions:"
            for i, action in enumerate(incident.recommended_actions[:5], 1):
                message += f"\n{i}. {action}"
...
"""
        ...
        <div class="section">
            ...
            <p style="white-space: pre-wrap;">{incident.root_cause_analysis[:2000]}</p>
        </div>
...
"""
        ...
        <div class="section">
            ...
            <p><a href="{incident.ticket_url}">{incident.ticket_key}</a></p>
        </div>
"""
...
ServiceNow Ticket: {incident.ticket_url or 'N/A'}
"""
